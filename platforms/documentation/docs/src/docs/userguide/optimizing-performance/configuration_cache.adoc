...
By default, Gradle automatically generates and manages the encryption key as a Java keystore stored under the Gradle User Home.

For environments where this is undesirable (for instance, because the `<<directory_layout.adoc#dir:gradle_user_home,GRADLE_USER_HOME>>` directory is shared across machines),
...
you must run Gradle while having the GRADLE_ENCRYPTION_KEY environment variable set with a valid AES 128-bit key.

There are many different ways of producing an encryption key that is compatible with Gradle.
One way is by using a command like this, which should work on Linux and Mac OS, or even on Windows, if using a tool like Cygwin:

```
❯ echo $(openssl enc -aes-128-cbc -k secret -P | grep key= | sed s/key=// | base64)
```

You can then use the key produced by that command (which is encoded as a Base64 string) and set it as the value of the
`GRADLE_ENCRYPTION_KEY` environment variable however you prefer configuring environment variables in your OS.
